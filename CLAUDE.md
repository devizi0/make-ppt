...
**대부분의 슬라이드는 bg-light/bg-dark를 쓴다. deco-right는 조건 충족 시 적극 활용한다.**

- **흰 배경 슬라이드 기본**: `bg-light-0.png` 또는 `bg-light-1.png` (opacity: 0.22~0.30)
- **다크 배경 슬라이드 기본**: `bg-dark-*.png` 중 하나 (opacity: 0.22~0.32)
- **`deco-right-blue/orange/purple.png` 사용 조건 — 3가지 모두 충족해야만 사용 가능**:
  1. 흰색 배경 슬라이드
  2. 모든 콘텐츠가 슬라이드 좌측에만 배치됨 (좌측 정렬 필수)
  3. 슬라이드 우측이 비어있음
  → 조건 충족 시 적극 활용할 것 — 콘텐츠를 좌측에 몰아 deco-right를 우측에 배치하는 레이아웃은 세련된 디자인 패턴이므로 적극 사용
  → 조건 불충족 시 bg-light-0/1 사용
